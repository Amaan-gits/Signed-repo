...
public class AppUsageCollector {
    ...
    public List<Map<String, Object>> getInstalledApps() {
        ...
        for (ApplicationInfo appInfo : packages) {
            ...
            try {
                app.put("versionName", packageManager.getPackageInfo(appInfo.packageName, 0).versionName);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    app.put("versionCode", packageManager.getPackageInfo(appInfo.packageName, 0).getLongVersionCode());
                } else {
                    app.put("versionCode", (long) packageManager.getPackageInfo(appInfo.packageName, 0).versionCode);
                }
                app.put("firstInstallTime", new Date(packageManager.getPackageInfo(appInfo.packageName, 0).firstInstallTime));
                app.put("lastUpdateTime", new Date(packageManager.getPackageInfo(appInfo.packageName, 0).lastUpdateTime));
            } catch (PackageManager.NameNotFoundException e) {
